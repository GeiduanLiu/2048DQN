...
Transition_dtype = np.dtype([('timestep', np.int32), ('state', np.uint8, (4, 4)), ('action', np.int32), ('reward', np.float32), ('nonterminal', np.bool_)])
blank_trans = (0, np.zeros((4, 4), dtype=np.uint8), 0, 0.0, False)
...
class ReplayMemory():
    ...
    def __next__(self):
        ...
        transitions[blank_mask] = blank_trans
        state = torch.tensor(transitions['state'], dtype=torch.uint8, device=self.device)  # Agent will turn into batch
        self.current_idx += 1
        return state
